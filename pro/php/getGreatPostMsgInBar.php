...
$watchType = $_GET['watchType'];
$everyPageItemNum = 3;
$dataIndex = $indexNum*$everyPageItemNum;
if($watchType == 0){
	$sql = "select * from posts where barBelong='$barName' and status = 1 and isBelongCompetitive = 1 ORDER BY createTime DESC limit $dataIndex,$everyPageItemNum";
	$sql2 = "select * from posts where barBelong='$barName' and status = 1 and isBelongCompetitive = 1";
}else{
	$sql = "select * from posts where barBelong='$barName' and status = 1 and isBelongCompetitive = 1 ORDER BY agreeNum DESC,createTime DESC limit $dataIndex,$everyPageItemNum";
	$sql2 = "select * from posts where barBelong='$barName' and status = 1 and isBelongCompetitive = 1";
}
